...
PointSampling::PointSampling(MeshCache &MC) : MCache(MC) {}
...
void PointSampling::Set(std::string method_id) {
  ...
  if (method == "Dijkstra") {
    printf("SampleMethod: Dijkstra\n");
  } else if (method == "GeodesicDistance") {
    printf("SampleMethod: GeodesicDistance\n");
  } else if (method == "RealDistance") {
    printf("SampleMethod: RealDistance\n");
  }
  std::default_random_engine e(static_cast<unsigned int>(time(NULL)));
  std::uniform_int_distribution<unsigned> u(0,
                                            static_cast<int>(MCache.NVertices));
...
void PointSampling::ComputeSamplesFromSelectedRegion(
    std::string sample_method, std::vector<int> &Region,
    std::vector<int> &SampleVertices, std::vector<int> &SampleEdges) {
  int s_p = -1;
  ...
  std::vector<int> v_p(MCache.NVertices, 0);
  std::vector<double> d(MCache.NEdges, DBL_MAX);
  Algorithm::Dijkstra_with_restrict(MCache, s_p, weight, v_p, d);
  int e_p = -1;
  if (sample_method == "RealDistance") {
    int farthestid = 0;
    double dist = 0.0;
    for (int j = 0; j < MCache.NVertices; j++) {
      if (Region[j] == 0)
    ...
    e_p = farthestid;
  } else if (sample_method == "Dijkstra") {
    double dist = 0.0;
...
double
PointSampling::PointSetRealDistance(const int &vid,
                                    const std::vector<int> &pointset) const {
  double dist = DBL_MAX;
...
double
PointSampling::PointSetPathLength(const int &vid,
                                  const std::vector<int> &pointset) const {
  double dist = DBL_MAX;
  for (const auto &id : pointset) {
