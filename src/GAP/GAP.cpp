...
GAP::GAP(Mesh& mesh, MeshCache& MCache):mesh(mesh),MCache(MCache)
{
	F_N = MCache.n_faces;
	V_N = MCache.n_vertices;
	area = MCache.fa;
	F0.resize(F_N);
	F1.resize(F_N);
	F2.resize(F_N);
...
void GAP::bpe_init()
{
	double area_sum = MCache.avg_fa;
	double area_same_factor = 1.0 / std::sqrt(area_sum);
	VV_ids.resize(V_N);
...
void GAP::GenFirstCut()
{
	...
	m2.close();
	*/
	meshcut = std::make_unique<MeshCut>(const_cast<Mesh&>(mesh),MCache);
	meshcut->SetCut(cv, ce);
	meshcut->MakeSeam();
	//meshcut->get_correspondence();
	Mesh tmp = meshcut->GetCutedMesh();
	//OpenMesh::IO::write_mesh(tmp,"tmp.obj");
}
