...
KPNewton::KPNewton(Mesh &m) : mesh(m), isbv(m.n_vertices(), 0) {
...
void KPNewton::PrepareDataFree(void) {
  ...
  ia.reserve(2 * nv + 1);
  ja.reserve(16 * nv);
  std::vector<std::vector<std::size_t>> tmp_ja(nv);

#pragma omp parallel for
  for (int vid = 0; vid < nv; vid++) {
    std::vector<std::size_t> rowid;
    ...
    std::sort(rowid.begin(), rowid.end(), std::less<std::size_t>());
...
#pragma omp parallel for
  ...
  std::vector<std::unordered_map<std::size_t, std::size_t>> spid(2 * nv);
//  size_t j = 0;
#pragma omp parallel for
...
#pragma omp parallel for
  ...
  assembleorder.clear();
  std::vector<std::vector<std::size_t>> tmp_assembleorder(tri.size());
#pragma omp parallel for
...
void KPNewton::RunFree(const EnergyType &etype) {
  ...
  std::vector<std::vector<double>> faceAngles =
      MeshAnglesFromFaceEdgeLen2(faceElens);

  std::vector<std::vector<std::complex<double>>> localframe(
      nF, std::vector<std::complex<double>>(3));
...
#pragma omp parallel for
  for (int i = 0; i < nF; i++) {
    ...
    // area[i] = 1;
    DC[i][0] =
        std::complex<double>(0.0, -0.25) * (frame[1] - frame[2]) / area[i];
    DC[i][1] =
        std::complex<double>(0.0, -0.25) * (frame[2] - frame[0]) / area[i];
    DC[i][2] =
        std::complex<double>(0.0, -0.25) * (frame[0] - frame[1]) / area[i];
  ...
  for (int iter = 0; iter < maxiter; iter++) {
    ...
    for (size_t fid = 0; fid < tri.size(); fid++) {
      ...
      e += etemp * area[fid];
      for (size_t i = 0; i < 3; i++) {
        auto gi = 2.0 * (alpha1 * DC[fid][i] * fz + alpha2 * D[fid][i] * fzb) *
                  area[fid];
        g[vhs[i]] += gi.real();
        g[vhs[i] + nV] += gi.imag();
      }
      { // fix alpha and beta
        ...
        if (y > 1e-50 && fabs(beta1 * beta2 * beta3) > 1e-50) {
          ...
          beta2 =
              (lambda3 * t1 * t1 + lambda4 * t2 * t2 - alpha2 * 2.0 / y) / 4.0;
          beta3 = (lambda3 * t1 + lambda4 * t2) / 4.0;
        }
      }

      double a1 =
          area[fid] * (2.0 * alpha1 + 4.0 * beta1 * fz.real() * fz.real());
      double a2 = area[fid] * 4.0 * beta1 * fz.real() * fz.imag();
      double a4 =
          area[fid] * (2.0 * alpha1 + 4.0 * beta1 * fz.imag() * fz.imag());
      ...
      double d1 =
          area[fid] * (2.0 * alpha2 + 4.0 * beta2 * fzb.real() * fzb.real());
      double d2 = area[fid] * 4.0 * beta2 * fzb.real() * fzb.imag();
      double d4 =
          area[fid] * (2.0 * alpha2 + 4.0 * beta2 * fzb.imag() * fzb.imag());
...
#pragma omp parallel for
    ...
    if (normg < 1e-4 || (e - newEnergy) / e < 1e-4 || std::isnan(newEnergy) ||
        std::isinf(newEnergy)) {
      break;
    }
  }
...
void KPNewton::ComputeMIPS(double &energy, double &alpha1, double &alpha2,
...
void KPNewton::ComputeAMIPS(double &energy, double &alpha1, double &alpha2,
                            double &beta1, double &beta2, double &beta3,
                            const double &x, const double &y) {
...
double KPNewton::ComputeTMax(const std::vector<std::complex<double>> &x,
                             const std::vector<std::complex<double>> &d) const {
  double temp_t = std::numeric_limits<double>::infinity();
  std::vector<double> a(tri.size()), b(tri.size()), c(tri.size()),
      b1(tri.size()), b2(tri.size()), tt(tri.size()), tt1(tri.size()),
      tt2(tri.size());
...
double KPNewton::ComputeEnergy(
    const std::vector<std::complex<double>> &pos,
...
std::vector<std::vector<double>> KPNewton::MeshAnglesFromFaceEdgeLen2(
    const std::vector<std::vector<double>> &len2) const {
  std::vector<std::vector<double>> ang;
  ang.resize(len2.size(), std::vector<double>(3));
#pragma omp parallel for
  for (int i = 0; i < len2.size(); i++) {
    ang[i] = {std::acos((len2[i][1] + len2[i][2] - len2[i][0]) /
                        std::sqrt(len2[i][1] * len2[i][2]) / 2.0),
              std::acos((len2[i][2] + len2[i][0] - len2[i][1]) /
                        std::sqrt(len2[i][2] * len2[i][0]) / 2.0),
              std::acos((len2[i][0] + len2[i][1] - len2[i][2]) /
                        std::sqrt(len2[i][0] * len2[i][1]) / 2.0)};
  }
  return ang;
}