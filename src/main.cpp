...
int main(int argc, char *argv[]) {
  ...
  Mcut2.SetBanCondition(PF.GetLocalMaximizer(), Mcut.GetCutvertex(),
                        opt.BanArea_Method);
  Mcut2.CalcBanArea(opt.BanArea_Dn, opt.BanArea_Metric, opt.BanArea_alpha,
                    opt.BanArea_ShrinkRate);
  std::vector<int> AllowedArea = Mcut2.GetMaxConnectedRegion();
  std::vector<int> cut2Edges, cut2Vertices;
  PS.ComputeSamplesFromSelectedRegion(opt.PS_method, AllowedArea, cut2Vertices,
                                      cut2Edges);
