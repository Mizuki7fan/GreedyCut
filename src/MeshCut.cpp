...
void MeshCut::Set(const std::vector<int> &lmk_,
                  const std::vector<int> &initseam_) {
  this->lmk = lmk_;
  this->initseam = initseam_;
}
...
void MeshCut::MakeSeam() {
  ...
  for (int i = 0; i < cutEdge.size(); i++) {
    Mesh::HalfedgeHandle he =
        oriMesh.halfedge_handle(oriMesh.edge_handle(cutEdge[i]), 0);
  ...
  for (int i = 0; i < uv_idx; i++) {
    cuted_mesh.add_vertex(oriMesh.point(
        oriMesh.vertex_handle(static_cast<unsigned int>(idx_to_mesh_idx[i]))));
  }
  for (const auto &fh : oriMesh.faces()) {
    std::vector<Mesh::VertexHandle> face_vhandles;
    for (const auto &fheh : oriMesh.fh_range(fh)) {
      face_vhandles.push_back(cuted_mesh.vertex_handle(
          static_cast<unsigned int>(he_to_idx[fheh.idx()])));
    }
    cuted_mesh.add_face(face_vhandles);
  }
}

void MeshCut::SetBanCondition(const std::vector<int> &banV,
...
void MeshCut::CalcBanArea(int Dn, std::string Metric, double Alpha,
  ...
  while (1) {
    BanV.clear();
    BanV.resize(MC.NVertices, 0);
    BanE.clear();
    BanE.resize(MC.NEdges, 0);
    BanF.clear();
    BanF.resize(MC.NFaces, 0);
    if (Metric == "Dijkstra") {
      ...
      for (int u = 0; u < ban_vertex.size(); u++) {
        ...
        while (1) {
          ...
          if (is_visited[tmp.id])
            continue;
          is_visited[tmp.id] = 1;
          for (std::size_t uu = 0; uu < MC.VV[tmp.id].size(); uu++) {
            int vid = MC.VV[tmp.id][uu];
            int eid = MC.VE[tmp.id][uu];
...
void MeshCut::GetCorrespondence(std::vector<std::size_t> &he2idx,
