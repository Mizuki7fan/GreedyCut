#include "PP.h"
//#include "..//MeshDefinition/Mesh_doubleIO.h"
...
Parafun2::Parafun2(string filename, string output_path)
{
...
Parafun2::Parafun2(Mesh& m,std::string s)
{
	mesh = m;
	F_N = mesh.n_faces();
	V_N = mesh.n_vertices();
...
bool Parafun2::writeobj(Mesh& out_mesh, const string out_filename)
{
	...
	//return Mesh_doubleIO::save_mesh(out_mesh, out_filename.c_str());
...
void Parafun2::run_bpe()
{
	init();
	BPE();
	writeobj(mesh, output_dir + "\\"+ "_pp_para.obj");
	//OpenMesh::IO::write_mesh(mesh, "C:\\meigyoku\\exp6\\Output\\alien\\_pp_para.obj");
}
...
void Parafun2::Update_source_same_t()
{
	double t_min = 1;
	int geqK = 0;
...
void Parafun2::max_step(const VectorXd& xx, const VectorXd& dd, double& step)
{
...
void Parafun2::calc_gradient_norm(const VectorXd& x)
{
	double area_now;
	int f0, f1, f2;
	double j00, j01, j10, j11;
...
void Parafun2::backtracking_line_search(const VectorXd& x, const VectorXd& d, const VectorXd& negetive_grad, double& alpha)
{
	double h = 0.5;
...
void Parafun2::Energy(const VectorXd& position, double& energyupdate)
{
	double energy = 0;
...
void Parafun2::local_coordinate_inverse(int i, double& p00, double& p01, double& p10, double& p11)
{
	int f0 = F0[i];
	int f1 = F1[i];
	int f2 = F2[i];
...
double Parafun2::newton_equation(const double& a, const double& b, const double& K)
{
	double tt = 1;
