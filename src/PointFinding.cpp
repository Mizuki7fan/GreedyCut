...
PointFinding::PointFinding(const Mesh &m, const Mesh &pm, MeshCache &MCache)
    : OriMesh(m), ParaedMesh(pm), MC(MCache) {
...
void PointFinding::Set(std::string metric) { this->Metric = metric; }
...
void PointFinding::PrepareComputeDistortion() {
  ...
  for (const auto &fh : OriMesh.faces()) {
    ...
    const auto &p2 = OriMesh.point(
        OriMesh.to_vertex_handle(OriMesh.next_halfedge_handle(heh)));
    auto np = (p1 - p0) % (p2 - p0);
...
void PointFinding::ComputeFaceDistortion() {
  ...
  for (const auto &fh : ParaedMesh.faces()) {
    ...
    const auto &p2 = ParaedMesh.point(
        ParaedMesh.to_vertex_handle(ParaedMesh.next_halfedge_handle(heh)));
    auto np = (p1 - p0) % (p2 - p0);
    // double area = np.norm() * 0.5;
    np.normalize();
    auto qx1 = (p1 - p0).norm();
    ...
    if (det > 0.0) {
      ...
      facedistortion[fid] = alpha * (a * a + b * b + c * c) / det * 0.5 +
                            (1 - alpha) * (det + 1.0 / det) * 0.5;
...
void PointFinding::FindByNeighbourhood(
  ...
  for (int i = 0; i < OriMesh.n_vertices(); i++) {
    ...
    while (1) {
      ...
      next_competitor.erase(
          std::unique(next_competitor.begin(), next_competitor.end()),
          next_competitor.end());
